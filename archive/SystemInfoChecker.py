...
def print_heading(f, heading):
    """
    Print a blank line and then the heading.
    """
    print_and_write(f, "")
    print_and_write(f, heading)
...
def print_windows_info(f):
    ...
    """
    try:
        print_heading(f, "=== CPU Information (Windows) ===")
        cpu_cmd = [
            'wmic', 'cpu', 'get',
    ...
    print_heading(f, "=== Memory Information (Windows) ===")
...
def print_linux_info(f):
    ...
    """
    try:
        print_heading(f, "=== CPU Information (Linux) ===")
        ...
        try:
            ...
            print_heading(f, "=== CPU Information (Linux - fallback) ===")
    ...
    print_heading(f, "=== Memory Information (Linux) ===")
...
def print_macos_info(f):
    ...
    """
    print_heading(f, "=== CPU Information (macOS) ===")
    try:
    ...
    print_heading(f, "=== Memory Information (macOS) ===")
    try:
...
def print_freebsd_info(f):
    print_heading(f, "=== CPU Information (FreeBSD) ===")
    try:
    ...
    try:
        ...
        pass
    print_and_write(f, f"Virtualization: {virtualization}")
    print_heading(f, "=== Memory Information (FreeBSD) ===")
    try:
...
def print_openbsd_info(f):
    print_heading(f, "=== CPU Information (OpenBSD) ===")
    try:
        model_str = subprocess.check_output(["sysctl", "-n", "hw.model"], universal_newlines=True).strip()
        print_and_write(f, f"CPU Name: {model_str}")
    except:
    ...
    print_heading(f, "=== Memory Information (OpenBSD) ===")
    try:
        mem_bytes = int(subprocess.check_output(["sysctl", "-n", "hw.physmem"], universal_newlines=True).strip())
        print_and_write(f, f"Total System RAM: {mem_bytes / (1024**3):.1f} GB")
    except:
...
def print_netbsd_info(f):
    print_heading(f, "=== CPU Information (NetBSD) ===")
    try:
    ...
    try:
        ...
        pass
    print_and_write(f, f"Virtualization: {virt_status}")
    print_heading(f, "=== Memory Information (NetBSD) ===")
    try:
...
def print_solaris_info(f):
    ...
    """
    print_heading(f, "=== CPU Information (Solaris) ===")
    try:
    ...
    print_heading(f, "=== Memory Information (Solaris) ===")
    try:
...
def print_aix_info(f):
    print_heading(f, "=== CPU Information (AIX) ===")
    try:
    ...
    print_heading(f, "=== Memory Information (AIX) ===")
    try:
...
def print_haiku_info(f):
    print_heading(f, "=== CPU Information (Haiku) ===")
    try:
    ...
    print_heading(f, "=== Memory Information (Haiku) ===")
    try:
...
def print_dragonfly_info(f):
    print_heading(f, "=== CPU Information (DragonFly BSD) ===")
    try:
        model_str = subprocess.check_output(["sysctl", "-n", "hw.model"], universal_newlines=True).strip()
        print_and_write(f, f"CPU Name: {model_str}")
    except:
    ...
    try:
        ...
        pass
    print_and_write(f, f"Virtualization: {virt_status}")
    print_heading(f, "=== Memory Information (DragonFly BSD) ===")
    try:
        mem_bytes = int(subprocess.check_output(["sysctl", "-n", "hw.physmem"], universal_newlines=True).strip())
        print_and_write(f, f"Total System RAM: {mem_bytes / (1024**3):.1f} GB")
    except:
...
def print_plan9_info(f):
    ...
    """
    print_heading(f, "=== CPU and Memory Information (Plan 9) ===")
    try:
...
def print_minix_info(f):
    ...
    """
    print_heading(f, "=== CPU and Memory Information (Minix) ===")
    try:
...
def print_android_info(f):
    ...
    """
    print_heading(f, "=== CPU and Memory Information (Android) ===")
...
def print_additional_info(f):
    print_heading(f, "=== Additional System Information ===")
    # System Architecture and Processor Details
    arch = platform.architecture()[0]
    machine = platform.machine()
    processor = platform.processor()
    print_and_write(f, f"System Architecture (bits): {arch}")
    print_and_write(f, f"Machine Type: {machine}")
    print_and_write(f, f"Processor Info: {processor}")

    print_heading(f, "=== Networking Information ===")
    try:
        hostname = socket.gethostname()
        print_and_write(f, f"Hostname: {hostname}")
        # Attempt to get a primary IP address
        ip_address = None
        try:
        ...
        print_and_write(f, f"Local IP Address: {ip_address}")
    except Exception as e:
        print_and_write(f, f"Networking information not available: {e}")

###############################################################################
# Programming Languages Environment
###############################################################################

def print_programming_languages_environment(f):
    print_heading(f, "=== Programming Languages Environment ===")
    # Define languages with their commands, including Python
    languages = [
        ("C (GCC)", ["gcc", "--version"]),
        ("C++ (G++)", ["g++", "--version"]),
        ("D (DMD)", ["dmd", "--version"]),
    ...
    # Sort languages lexicographically by name.
    languages.sort(key=lambda x: x[0])
    
    for lang, cmd in languages:
        # Check if the language's binary exists; if not, skip it.
        binary_path = shutil.which(cmd[0])
        if not binary_path:
            continue
        print_and_write(f, "")
        print_and_write(f, f"{lang}:")
        try:
            ...
            if out:
                ...
                version_line = "No version info available"
        except Exception:
            continue  # Skip if an error occurs
        print_and_write(f, f"  Version: {version_line}")
        print_and_write(f, f"  Path: {binary_path}")

###############################################################################
# Locale and Encoding Information
###############################################################################

def print_locale_and_encoding_info(f):
    print_heading(f, "=== Locale and Encoding Information ===")
    # Use getlocale() instead of deprecated getdefaultlocale()
    default_locale = locale.getlocale()
    preferred_encoding = locale.getpreferredencoding()
    print_and_write(f, f"Default Locale: {default_locale}")
    print_and_write(f, f"Preferred Encoding: {preferred_encoding}")
    try:
        locale_output = subprocess.check_output(["locale"], universal_newlines=True, stderr=subprocess.DEVNULL)
        print_and_write(f, "")
        print_and_write(f, "Locale Command Output:")
        print_and_write(f, locale_output.strip())
    except Exception:
        print_and_write(f, "Locale command not available.")
...
def main():
    ...
    # Append additional system information, programming languages info, and locale info
    print_additional_info(output)
    print_programming_languages_environment(output)
    print_locale_and_encoding_info(output)
